...
export class UserService {
  ...
  getUserById(userId: number): Observable<User> {
    return this.http
      .get<User>(this.url + 'api/users/' + userId, this.getHttpOptions())
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
  ...
  addFriend(userId: number, user: User): Observable<User> {
    ...
    return this.http
      .post<User>(
        this.url + 'api/users/friends/' + userId,
        user,
        this.getHttpOptions()
      )
      .pipe(
  ...
  updateEmail(user: User, email: string): Observable<User> {
    return this.http
      .put<User>(
        this.url + 'api/users/' + user.id + '/' + 'email',
        email,
        this.getHttpOptions()
      )
      .pipe(
  ...
  disableUser(user: User): Observable<User> {
    ...
    return this.http
      .put<User>(
        this.url + 'api/users/' + user.id + '/archive',
        user,
        this.getHttpOptions()
      )
      .pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () =>
              new Error(
                'UserService.disableUser(): error disabling User: ' + err
              )
          );
        })
      );
  ...
  enableUser(user: User): Observable<User> {
    ...
    return this.http
      .put<User>(
        this.url + 'api/users/' + user.id + '/unarchive',
        user,
        this.getHttpOptions()
      )
      .pipe(
